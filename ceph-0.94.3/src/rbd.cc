...
static int do_limit(librbd::Image& image, librados::IoCtx& io_ctx,
                     const char* poolname, const char *imgname, uint64_t iops_write,uint64_t iops_read
                     , uint64_t bps_write,uint64_t bps_read,uint64_t bps_total,uint64_t iops_total,int *order)
{
	   char iops_write_buf[64];
	   char iops_read_buf[64];
	   char bps_write_buf[64];
	   char bps_read_buf[64];
	   char bps_total_buf[64];
	   char iops_total_buf[64];
	   memset(iops_write_buf,0,64);
	   memset(iops_read_buf,0,64);
	   memset(bps_write_buf,0,64);
	   memset(bps_read_buf,0,64);
	   memset(bps_total_buf,0,64);
	   memset(iops_total_buf,0,64);
	   int err;
	   //string config_oid= string(imgname)+string(".rbd");
	   string config_oid=image.get_header_id();
	   if(iops_write>0)
	   {
		   sprintf(iops_write_buf,"%llu",iops_write);
		   librados::bufferlist iops_write_bl;
	  		iops_write_bl.append(iops_write_buf,strlen(iops_write_buf)+1);
	  		io_ctx.setxattr(config_oid,  "iops_write", iops_write_bl);
	   }
	   if(iops_read>0)
	   {
		   sprintf(iops_read_buf,"%llu",iops_read);
		   librados::bufferlist iops_read_bl;
	  		iops_read_bl.append(iops_read_buf,strlen(iops_read_buf)+1);
	  		io_ctx.setxattr(config_oid,  "iops_read", iops_read_bl);
	   }
	   if(bps_write>0)
	   {
			sprintf(bps_write_buf,"%llu",bps_write);
			librados::bufferlist bps_write_bl;
  			bps_write_bl.append(bps_write_buf,strlen(bps_write_buf)+1);
  			io_ctx.setxattr(config_oid,  "bps_write", bps_write_bl);
	   	}
	   if(bps_read>0)
	   {
			sprintf(bps_read_buf,"%llu",bps_read);
			librados::bufferlist bps_read_bl;
  			bps_read_bl.append(bps_read_buf,strlen(bps_read_buf)+1);
  			io_ctx.setxattr(config_oid,  "bps_read", bps_read_bl);
	   }
	   if(bps_total>0)
	   {
			sprintf(bps_total_buf,"%llu",bps_total);
			librados::bufferlist bps_total_bl;
  			bps_total_bl.append(bps_total_buf,strlen(bps_total_buf)+1);
  			io_ctx.setxattr(config_oid,  "bps_total", bps_total_bl);
	   }
	   if(iops_total>0)
	   {
			sprintf(iops_total_buf,"%llu",iops_total);
			librados::bufferlist iops_total_bl;
  			iops_total_bl.append(iops_total_buf,strlen(iops_total_buf) + 1);
  			io_ctx.setxattr(config_oid,  "iops_total", iops_total_bl);
	   }
		librados::bufferlist bl;
		io_ctx.notify(config_oid, 0, bl);
      return 0;
}
static int do_show_limits(librbd::Image& image,librados::IoCtx& io_ctx,const char* imgname,char* imgmeta_mark)
{
 	  //string config_oid= string(imgname)+string(".rbd");
 	  string config_oid=image.get_header_id();
	  if(strcmp(imgmeta_mark,"bps_write")==0)
	  {
		  librados::bufferlist bps_write_res;
	  	  io_ctx.getxattr(config_oid,  "bps_write", bps_write_res);
		  printf("%s\n",bps_write_res.c_str());
	  }
	  if(strcmp(imgmeta_mark,"bps_read")==0)
	  {
		  librados::bufferlist bps_read_res;
	  	  io_ctx.getxattr(config_oid,  "bps_read", bps_read_res);
		  printf("%s\n",bps_read_res.c_str());
	  }
	  if(strcmp(imgmeta_mark,"iops_write")==0)
	  {
		  librados::bufferlist iops_write_res;
	  	  io_ctx.getxattr(config_oid,  "iops_write", iops_write_res);
		  printf("%s\n",iops_write_res.c_str());
	  }
	  if(strcmp(imgmeta_mark,"iops_read")==0)
	  {
		  librados::bufferlist iops_read_res;
	  	  io_ctx.getxattr(config_oid,  "iops_read", iops_read_res);
		  printf("%s\n",iops_read_res.c_str());
	  }
	  if(strcmp(imgmeta_mark,"bps_total")==0)
	  {
		  librados::bufferlist bps_total_res;
		  io_ctx.getxattr(config_oid,  "bps_total", bps_total_res);
		  printf("%s\n",bps_total_res.c_str());
	  }
	  if(strcmp(imgmeta_mark,"iops_total")==0)
	  {
		  librados::bufferlist iops_total_res;
	  	  io_ctx.getxattr(config_oid,  "iops_total", iops_total_res);
		  printf("%s\n",iops_total_res.c_str());
	  }
	  return 0;

}
static int do_list_imgmate_attr(librbd::Image& image,librados::IoCtx& io_ctx, const char *imgname)
{
	  //string config_oid= string(imgname)+string(".rbd");
	  string config_oid=image.get_header_id();
	  librados::bufferlist bps_write_res;
	  ...
	  std::cout<<"conf_rbd_throttle_bps_write:"<<atoi(bps_write_res.c_str())<<std::endl;
	  std::cout<<"conf_rbd_throttle_bps_read:"<<atoi(bps_read_res.c_str())<<std::endl;
	  std::cout<<"conf_rbd_throttle_iops_write:"<<atoi(iops_write_res.c_str())<<std::endl;
	  std::cout<<"conf_rbd_throttle_iops_read:"<<atoi(iops_read_res.c_str())<<std::endl;
	  std::cout<<"conf_rbd_throttle_bps_total:"<<atoi(bps_total_res.c_str())<<std::endl;
	  std::cout<<"conf_rbd_throttle_iops_total:"<<atoi(iops_total_res.c_str())<<std::endl;
	  	return 0;
}

static int do_rm_limit(librbd::Image& image,librados::IoCtx& io_ctx, const char *imgname,char* imgmeta_mark)
{
	//string config_oid= string(imgname)+string(".rbd");1
	string config_oid=image.get_header_id();
	if(strcmp(imgmeta_mark,"bps_write")==0)
	{
	  	bufferlist bps_write_bl;
		bps_write_bl.append("1000000000");
		io_ctx.setxattr(config_oid,	"bps_write", bps_write_bl);
	}
    if(strcmp(imgmeta_mark,"bps_read")==0)
	{
		bufferlist bps_read_bl;
		bps_read_bl.append("1000000000");
		io_ctx.setxattr(config_oid,	"bps_read", bps_read_bl);
    }
	if(strcmp(imgmeta_mark,"iops_write")==0)
	{
		bufferlist iops_write_bl;
		iops_write_bl.append("1000000000");
		io_ctx.setxattr(config_oid,	"iops_write", iops_write_bl);
    }
	if(strcmp(imgmeta_mark,"iops_read")==0)
	{
		bufferlist iops_read_bl;
		iops_read_bl.append("1000000000");
		io_ctx.setxattr(config_oid,	"iops_read", iops_read_bl);
	}
	if(strcmp(imgmeta_mark,"bps_total")==0)
	{
		bufferlist bps_total_bl;
		bps_total_bl.append("1000000000");
		io_ctx.setxattr(config_oid,	"bps_total", bps_total_bl);
	}
	if(strcmp(imgmeta_mark,"iops_total")==0)
	{
		bufferlist iops_total_bl;
		iops_total_bl.append("1000000000");
		io_ctx.setxattr(config_oid,	"iops_total", iops_total_bl);
	}
	return 0;
}
...
//shiqi add
static int get_img_cmd(const char *cmd)
{
  	  if (strcmp(cmd, "set")==0)
      	return OPT_LIMIT;
	  else if (strcmp(cmd, "get")==0)
      	return OPT_LIMIT_INFO;
	  else if (strcmp(cmd, "remove")==0)
      	return OPT_RM_LIMIT;
	  else if (strcmp(cmd, "list")==0)
      	return OPT_LIST_ATTR;
	  else
	  	return OPT_NO_CMD;	
}
...
//zhanglei add 
static bool inline CHECK_IF_NUM(const char* szString)
{
        const char* temp = szString;
	const char* p = NULL;
	while('\0' != (p = temp++)[0])
		if(p[0] < '0' || p[0] > '9')
			return false;
...
int main(int argc, const char **argv)
{
  ...
  long long max_iops=0;
  long long limit_iops=0;
  long long iops_write=0;
  long long iops_read=0;
  long long bps_write=0;
  long long bps_read=0;
  long long bps_total=0;
  long long iops_total=0;
  bool has_set_value=false;
  char img_meta_mark[32];
  memset(img_meta_mark,0,32);
  int order = 0;
  ...
  for (i = args.begin(); i != args.end(); ) {
    if (ceph_argparse_double_dash(args, i)) {
	  ...
	  //shiqi add
	  else if (ceph_argparse_flag(args, i, "conf_rbd_throttle_iops_write", (char*)NULL)) {
	  	if((i)!=args.end())
	  		{
	  		    //zhanglei add
	  		    if(false ==  CHECK_IF_NUM(*(i)))
	  		    {
	  			    cerr << "conf_rbd_throttle_iops_write must be number and >0" << std::endl;
			        return EXIT_FAILURE;
	  		    }

	  			iops_write=atoll(*(i));
				i=args.erase(i);
				if(iops_write<=0||iops_write>1000000000)
					{
						cerr << "conf_rbd_throttle_iops_write value must be >0 and <1000000000" << std::endl;
						return EXIT_FAILURE;
					}
				has_set_value=true;
	  		}
			strcpy(img_meta_mark,"iops_write");
    	}
	   else if (ceph_argparse_flag(args, i, "conf_rbd_throttle_iops_read", (char*)NULL)) {
	  	if((i)!=args.end())
	  		{
	  		    //zhanglei add
	  		    if(false ==  CHECK_IF_NUM(*(i)))
	  		    {
	  			    cerr << "conf_rbd_throttle_iops_read must be number and >0" << std::endl;
			        return EXIT_FAILURE;
	  		    }
								
	  			iops_read=atoll(*(i));
				i=args.erase(i);
				if(iops_read<=0||iops_read>1000000000)
					{
						cerr << "conf_rbd_throttle_iops_read value must be >0 and <1000000000" << std::endl;
						return EXIT_FAILURE;
					}
				has_set_value=true;
	  		}
			strcpy(img_meta_mark,"iops_read");
    	}
	    else if (ceph_argparse_flag(args, i, "conf_rbd_throttle_bps_write", (char*)NULL)) {
	  	if((i)!=args.end())
	  		{
	  		    //zhanglei add
	  		    if(false ==  CHECK_IF_NUM(*(i)))
	  		    {
	  			    cerr << "conf_rbd_throttle_bps_write must be number and >0" << std::endl;
			        return EXIT_FAILURE;
	  		    }
	  			bps_write=atoll(*(i));
				i=args.erase(i);
				if(bps_write<=0||bps_write>1000000000)
					{
						cerr << "conf_rbd_throttle_bps_write value must be >0 and <1000000000" << std::endl;
						return EXIT_FAILURE;
					}
				has_set_value=true;
	  		}
			strcpy(img_meta_mark,"bps_write");
    	}
		 else if (ceph_argparse_flag(args, i, "conf_rbd_throttle_bps_read", (char*)NULL)) {
	  	if((i)!=args.end())
	  		{
	  		    //zhanglei add
	  		    if(false ==  CHECK_IF_NUM(*(i)))
	  		    {
	  			    cerr << "conf_rbd_throttle_bps_read must be number and >0" << std::endl;
			        return EXIT_FAILURE;
	  		    }
	  			bps_read=atoll(*(i));
				i=args.erase(i);
				if(bps_read<=0||bps_read>1000000000)
					{
						cerr << "conf_rbd_throttle_bps_read value must be >0 and <1000000000" << std::endl;
						return EXIT_FAILURE;
					}
				has_set_value=true;
	  		}
			strcpy(img_meta_mark,"bps_read");
    	}
		  else if (ceph_argparse_flag(args, i, "conf_rbd_throttle_bps_total", (char*)NULL)) {
	  	if((i)!=args.end())
	  		{
	  		    //zhanglei add
	  		    if(false ==  CHECK_IF_NUM(*(i)))
	  		    {
	  			    cerr << "conf_rbd_throttle_bps_total must be number and >0" << std::endl;
			        return EXIT_FAILURE;
	  		    }
	  			bps_total=atoll(*(i));
				i=args.erase(i);
				if(bps_total<=0||bps_total>1000000000)
					{
						cerr << "conf_rbd_throttle_bps_total value must be >0 and <1000000000" << std::endl;
						return EXIT_FAILURE;
					}
				has_set_value=true;
	  		}
			strcpy(img_meta_mark,"bps_total");
    	}
		   else if (ceph_argparse_flag(args, i, "conf_rbd_throttle_iops_total", (char*)NULL)) {
	  	if((i)!=args.end())
	  		{
	  		    //zhanglei add
	  		    if(false ==  CHECK_IF_NUM(*(i)))
	  		    {
	  			    cerr << "conf_rbd_throttle_iops_total must be number and >0" << std::endl;
			        return EXIT_FAILURE;
	  		    }
	  			iops_total=atoll(*(i));
				i=args.erase(i);
				if(iops_total<=0||iops_total>1000000000)
					{
						cerr << "conf_rbd_throttle_iops_total value must be >0 and <1000000000" << std::endl;
						return EXIT_FAILURE;
					}
				has_set_value=true;
	  		}
			strcpy(img_meta_mark,"iops_total");
	  //shiqi add
  ...
  if (i == args.end()) {
    ...
    i = args.erase(i);
    if (i == args.end()) {
      cerr << "rbd: which image-meta set command do you want?" << std::endl;
      return EXIT_FAILURE;
    }
    opt_cmd = get_img_cmd(*i);
	//shiqi add
...
if (!set_conf_param(v, p1, p2, p3)) { \
  ...
  if (imgname && talk_to_cluster &&
      (opt_cmd == OPT_RESIZE || opt_cmd == OPT_SNAP_CREATE ||
       ...
       opt_cmd == OPT_STATUS
       ||opt_cmd==OPT_LIMIT||opt_cmd==OPT_LIMIT_INFO||opt_cmd==OPT_LIST_ATTR||opt_cmd==OPT_RM_LIMIT //shiqi add
       )) {
  ...
  case OPT_LIMIT:
  	if(!has_set_value)
  	{
  		cerr << "please give a value" << std::endl;
  		return EXIT_FAILURE;
  	}
    r= do_limit(image,io_ctx,poolname,imgname,iops_write,iops_read,bps_write,bps_read,bps_total,iops_total,&order);
     if (r < 0) {
      cerr << "rbd: limit error: " << cpp_strerror(-r) << std::endl;
      return -r;
    }
    break;

  case OPT_LIMIT_INFO:
	r=do_show_limits(image,io_ctx,imgname,img_meta_mark);
	break;
  case OPT_LIST_ATTR:
  	r=do_list_imgmate_attr(image,io_ctx,imgname);
  	break;
  case OPT_RM_LIMIT:
  	r=do_rm_limit(image,io_ctx,imgname,img_meta_mark);
	break;
  case OPT_CLONE:
