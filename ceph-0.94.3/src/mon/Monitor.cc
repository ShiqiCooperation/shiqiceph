...
int tds_warning;
int src_osd_num,dest_osd_num;
int src_capacity,dest_capacity;
                                    ...
                                    Formatter *f)
{
  ...
  if(tds_warning>0)
   {
      if(src_osd_num>dest_osd_num && src_capacity>(dest_capacity*1024))
      {
          sprintf(warn_detail,"ceph node num is %d,limited is %d.ceph capacity is %d MB,limited is %d MB.warning times %d\n",src_osd_num,dest_osd_num,src_capacity,dest_capacity*1024,tds_warning);
      }
	  else if(src_osd_num>dest_osd_num)
	  {	  	
		  sprintf(warn_detail,"ceph node num is %d,limited is %d.warning times %d\n",src_osd_num,dest_osd_num,tds_warning);
	  }
	  else if(src_capacity>dest_capacity)
	  {			
		  sprintf(warn_detail,"ceph capacity is %d MB,limited is %d MB.warning times %d\n",src_capacity,dest_capacity*1024,tds_warning);
	  }
	  else if(dest_osd_num==0&&dest_capacity==0)
	  {
	  	  sprintf(warn_detail,"mon started! warning times %d\n",tds_warning);
		  //std::cerr<<"get in warning num==0 capacity=0 "<<dest_osd_num<<","<<dest_capacity<<std::endl;
	  }
	  else
	  {
		  sprintf(warn_detail,"none define error.warning times %d\n",tds_warning);
	  }
   	  //std::cerr<<"get out warning num==0 capacity=0 "<<dest_osd_num<<","<<dest_capacity<<std::endl;
   	  //fss<<"\n            "<<warn_detail;
   	  status.push_back(warn_detail);
   }
  //std::cerr<<"get out out warning num==0 capacity=0 "<<dest_osd_num<<","<<dest_capacity<<std::endl;
  //shiqi add
  //status.push_front(fss.str());
  if (f)
...
void Monitor::get_cluster_status(stringstream &ss, Formatter *f)
{
  ...
  if (f) {
    ...
    ss << "     health " << joinify(health.begin(), health.end(), //shiqi 
				    string("\n            ")) << "\n";
    ss << "     monmap " << *monmap << "\n";
...
void Monitor::handle_command(MMonCommand *m)
{
  ...
  if (prefix == "compact") {
	...
	//shiqi add
  } else if (prefix == "invalid_mon" ||
	     prefix == "health" ||
	     prefix == "df") {
    string detail;
    cmd_getval(g_ceph_context, cmdmap, "detail", detail);

    if (prefix == "invalid_mon") {
