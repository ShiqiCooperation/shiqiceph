...
#include <errno.h>
#include <limits.h>
#include <stdio.h> //shiqi add
#include <stdlib.h> //shiqi add
#include <fcntl.h>//shiqi add
#include <string.h>//shiqi add
...
int invoke_async_request(ImageCtx *ictx, const std::string& request_type,
  ...
  void image_info(ImageCtx *ictx, image_info_t& info, size_t infosize)
  {
	...
	//shiqi add
	/*fo.bps_write_limit=ictx->bps_write_limit;
	info.bps_read_limit=ictx->bps_read_limit;
	info.iops_write_limit=ictx->iops_write_limit;
	info.iops_read_limit=ictx->iops_read_limit;
	info.bps_total_limit=ictx->bps_total_limit;
	info.iops_total_limit=ictx->iops_total_limit;*/
	//shiqi add
...
reprotect_and_return_err:
		...
		uint64_t size, int order)
  {
	...
	io_ctx.setxattr(header_oid,	"bps_write", bps_write_bl);

	bufferlist bps_read_bl;
	bps_read_bl.append("1000000000");
	io_ctx.setxattr(header_oid,	"bps_read", bps_read_bl);

	bufferlist iops_write_bl;
	iops_write_bl.append("1000000000");
	io_ctx.setxattr(header_oid,	"iops_write", iops_write_bl);

	bufferlist iops_read_bl;
	iops_read_bl.append("1000000000");
	io_ctx.setxattr(header_oid,	"iops_read", iops_read_bl);

	bufferlist bps_total_bl;
	bps_total_bl.append("1000000000");
	io_ctx.setxattr(header_oid,	"bps_total", bps_total_bl);

	bufferlist iops_total_bl;
	iops_total_bl.append("1000000000");
	io_ctx.setxattr(header_oid,	"iops_total", iops_total_bl);
  ...
  int ictx_refresh(ImageCtx *ictx)
  {
	...
	if (ictx->old_format) {
	  ...
	  //shiqi add
	  string config_oid = old_header_name(ictx->name);
	  librados::bufferlist bps_write_res;
  	  ictx->data_ctx.getxattr(config_oid,  "bps_write", bps_write_res);
  	  //std::cout<<  "Object xattr: "  << flow_res.c_str()  << std::endl ;

	  librados::bufferlist bps_read_res;
  	  ictx->data_ctx.getxattr(config_oid,  "bps_read", bps_read_res);

	  librados::bufferlist iops_write_res;
  	  ictx->data_ctx.getxattr(config_oid,  "iops_write", iops_write_res);

	  librados::bufferlist iops_read_res;
  	  ictx->data_ctx.getxattr(config_oid,  "iops_read", iops_read_res);

	  librados::bufferlist bps_total_res;
	  ictx->data_ctx.getxattr(config_oid,  "bps_total", bps_total_res);

	  librados::bufferlist iops_total_res;
  	  ictx->data_ctx.getxattr(config_oid,  "iops_total", iops_total_res);
  	  //std::cout<<  "Object xattr: "  << iops_res.c_str()  << std::endl ;
	  //shiqi add
	  ...
	  ictx->init_layout();
	  //shiqi add
	  ictx->bps_write_limit=atoi(bps_write_res.c_str());
	  ictx->bps_read_limit=atoi(bps_read_res.c_str());
	  ictx->iops_write_limit=atoi(iops_write_res.c_str());
	  ictx->iops_read_limit=atoi(iops_read_res.c_str());
	  ictx->bps_total_limit=atoi(bps_total_res.c_str());
	  ictx->iops_total_limit=atoi(iops_total_res.c_str());
	  //shiqi add
	} else {
	  do {
  ...
  long current_time;
  long long temp_iops;
  pthread_mutex_lock(&ictx->wtlock);//私有锁开始
  
	  current_time=time(NULL);//--------------------------------------------------iops限制统计
	  if(0==ictx->iops_write_last_time)
	  {
	  		times_tamp=1;
	  }
	  else
	  {
	  	times_tamp=current_time-ictx->iops_write_last_time;
	  }
	  ictx->iops_write_last_time=current_time;
	  temp_iops=ictx->iops_rwbucket+ictx->iops_write_limit*times_tamp; //iops limits
	  if(temp_iops<ictx->iops_bucket_wr_max)
		 ictx->iops_rwbucket=temp_iops;
	  else
		 ictx->iops_rwbucket=ictx->iops_bucket_wr_max;
	  while(ictx->iops_rwbucket<1)
	  {
	  	sleep(1);
		current_time=time(NULL);
		times_tamp=current_time-ictx->iops_write_last_time;
		ictx->iops_write_last_time=current_time;
		temp_iops=ictx->iops_rwbucket+ictx->iops_write_limit*times_tamp;
		if(temp_iops<ictx->iops_bucket_wr_max)
				ictx->iops_rwbucket=temp_iops;
			else
			  ictx->iops_rwbucket=ictx->iops_bucket_wr_max;
	  }
	  ictx->iops_rwbucket--;//------------------------------------------------iops限制统计结束


	//new add
	current_time=time(NULL);//--------------------------------------------------混合读写iops限制统计
	  if(0==ictx->total_iops_lasttime)
	  {
	  		times_tamp=1;
	  }
	  else
	  {
	  	times_tamp=current_time-ictx->total_iops_lasttime;
	  }
	  ictx->total_iops_lasttime=current_time;
	  temp_iops=ictx->total_iops_bucket+ictx->iops_total_limit*times_tamp; //iops limits
	  if(temp_iops<ictx->total_iops_bucket_max)
		 ictx->total_iops_bucket=temp_iops;
	  else
		 ictx->total_iops_bucket=ictx->total_iops_bucket_max;
	  while(ictx->total_iops_bucket<1)
	  {
	  	sleep(1);
		current_time=time(NULL);
		times_tamp=current_time-ictx->total_iops_lasttime;
		ictx->total_iops_lasttime=current_time;
		temp_iops=ictx->total_iops_bucket+ictx->iops_total_limit*times_tamp;
		if(temp_iops<ictx->total_iops_bucket_max)
				ictx->total_iops_bucket=temp_iops;
			else
			  ictx->total_iops_bucket=ictx->total_iops_bucket_max;
	  }
	  ictx->total_iops_bucket--;//------------------------------------------------混合读写iops限制统计结束*/
	//new add
	  
  current_time=time(NULL);//-------------------------------------------------------流量限制
  if(0==ictx->last_time)
  {
  		times_tamp=1;
  }
  ...
  {
  	times_tamp=current_time-ictx->last_time;
  }
  ictx->last_time=current_time;
  temp=ictx->bucket+ictx->bps_write_limit*times_tamp;
  if(temp<ictx->bps_bucket_wr_max)
	 ictx->bucket=temp;
  else
	 ictx->bucket=ictx->bps_bucket_wr_max;
  while(ictx->bucket<=len)//-------------------------------令牌桶算法限制流量大小
  {
		sleep(1);
		current_time=time(NULL);
		times_tamp=current_time-ictx->last_time;
		...
		if(temp<ictx->bps_bucket_wr_max)
		  ...
		  ictx->bucket=ictx->bps_bucket_wr_max;//--------------------------------------流量限制结束
  }
  ictx->bucket-=len;

   //new add
	  current_time=time(NULL); //--------------------------------------混合读写流量限制
	  if(0==ictx->total_bps_lasttime)
	  {
	  		times_tamp=1;
	  }
	  else
	  {
	  	times_tamp=current_time-ictx->total_bps_lasttime;
	  }
	  ictx->total_bps_lasttime=current_time;
	  temp=ictx->total_bps_bucket+ictx->bps_total_limit*times_tamp;
	  if(temp<ictx->total_bps_bucket_max)
		 ictx->total_bps_bucket=temp;
	  else
		 ictx->total_bps_bucket=ictx->total_bps_bucket_max;
	  while(ictx->total_bps_bucket<=len)
	  {
			sleep(1);
			current_time=time(NULL);
			//把当前时间设置为last时间
	  ...
	  ictx->total_bps_bucket-=len;//---------------------------------混合读写流向限制结束
	  //new add
  pthread_mutex_unlock(&ictx->wtlock);//锁结束
    //shiqi add //////////////////////////////////////////////////////////////////////////////
	        ...
	        char *buf, bufferlist *pbl, AioCompletion *c, int op_flags)
  {
	 ...
	 //char data2[64];
	 //memset(data,'a',4);
	   //sprintf(data,"bucket is %lld,last_time is %ld,max_limit is %lld\n",ictx->iops_rdbucket,ictx->iops_read_last_time,ictx->limit_iops);
	    //::write(handle,data,strlen(data));
	  pthread_mutex_lock(&ictx->rdlock);
	  
	  //flow limited
	  current_time=time(NULL);
	  if(0==ictx->iops_read_last_time)
	  {
	  		times_tamp=1;
	  }
	  else
	  {
	  	times_tamp=current_time-ictx->iops_read_last_time;
	  }
	  ictx->iops_read_last_time=current_time;
	  temp_iops=ictx->iops_rdbucket+ictx->iops_read_limit*times_tamp; //iops limits
	  if(temp_iops<ictx->iops_bucket_rd_max)
		 ictx->iops_rdbucket=temp_iops;
	  else
		 ictx->iops_rdbucket=ictx->iops_bucket_rd_max;
	  while(ictx->iops_rdbucket<1)
	  {
	  	sleep(1);
		current_time=time(NULL);
		times_tamp=current_time-ictx->iops_read_last_time;
		ictx->iops_read_last_time=current_time;
		temp_iops=ictx->iops_rdbucket+ictx->iops_read_limit*times_tamp;
		//sprintf(data2,"temp_iops is%lld,limit_iops is %lld,times_tamp is %ld\n",temp_iops,ictx->limit_iops,times_tamp);
		//::write(handle,data2,strlen(data));
		if(temp_iops<ictx->iops_bucket_rd_max)
				ictx->iops_rdbucket=temp_iops;
			else
			  ictx->iops_rdbucket=ictx->iops_bucket_rd_max;
	...
	//new add
	  ...
	  current_time=time(NULL);
	  if(0==ictx->read_last_time)
	  {
	  		times_tamp=1;
	  }
	  else
	  {
	  	times_tamp=current_time-ictx->read_last_time;
	  }
	  ictx->read_last_time=current_time;
	  temp=ictx->rdbucket+ictx->bps_read_limit*times_tamp;
	  if(temp<ictx->bps_bucket_rd_max)
		 ictx->rdbucket=temp;
	  else
		 ictx->rdbucket=ictx->bps_bucket_rd_max;
	  while(ictx->rdbucket<=image_extents[0].second)
	  {
			sleep(1);
			current_time=time(NULL);
			//把当前时间设置为last时间
	  ...
	  ictx->rdbucket-=image_extents[0].second;
	  ...
	  if(temp<ictx->total_bps_bucket_max)
		 ...
		 ictx->total_bps_bucket=ictx->total_bps_bucket_max;
	  while(ictx->total_bps_bucket<=image_extents[0].second)
	  {
			sleep(1);
			current_time=time(NULL);
			//把当前时间设置为last时间
	  ...
	  ictx->total_bps_bucket-=image_extents[0].second;
	  //new add
	  pthread_mutex_unlock(&ictx->rdlock);
	 }
     //shiqi add///////////////////////////////////////////////////////////////////////////////////////////
