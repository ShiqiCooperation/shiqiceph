...
extern int handle;
//int fd;
//static pthread_mutex_t tdslock;
//static pthread_mutex_t rdlock;
...
namespace librbd {
  ...
  */
  ...
  std::string Image::get_header_id()
  {
  	  ImageCtx *ictx = (ImageCtx *)ctx;
	  string x=ictx->header_oid;
	  return x;
  }
...
static void watch_notify_test_cb(uint8_t opcode, uint64_t ver, void *arg)
{
	  ...
	  //string config_oid= string(ictx->name)+string(".rbd");
	  librados::bufferlist bps_write_res;
  	  ictx->data_ctx.getxattr(ictx->header_oid,  "bps_write", bps_write_res);

	  librados::bufferlist bps_read_res;
  	  ictx->data_ctx.getxattr(ictx->header_oid,  "bps_read", bps_read_res);

	  librados::bufferlist iops_write_res;
  	  ictx->data_ctx.getxattr(ictx->header_oid,  "iops_write", iops_write_res);

	  librados::bufferlist iops_read_res;
  	  ictx->data_ctx.getxattr(ictx->header_oid,  "iops_read", iops_read_res);

	  librados::bufferlist bps_total_res;
  	  ictx->data_ctx.getxattr(ictx->header_oid,  "bps_total", bps_total_res);

	  
	  librados::bufferlist iops_total_res;
	  ictx->data_ctx.getxattr(ictx->header_oid,  "iops_total", iops_total_res);

      ictx->bps_write_limit=atoi(bps_write_res.c_str());
	  ictx->bps_bucket_wr_max=ictx->bps_write_limit*1.5;
	  
	  ictx->bps_read_limit=atoi(bps_read_res.c_str());
	  ictx->bps_bucket_rd_max=ictx->bps_read_limit*1.5;
	  
	  ictx->iops_write_limit=atoi(iops_write_res.c_str());
	  ictx->iops_bucket_wr_max=ictx->iops_write_limit*1.5;
	  	
	  ictx->iops_read_limit=atoi(iops_read_res.c_str());
	  ictx->iops_bucket_rd_max=ictx->iops_read_limit*1.5;
	  
      ictx->bps_total_limit=atoi(bps_total_res.c_str());
	  ictx->total_bps_bucket_max=ictx->bps_total_limit*1.5;
	  
	  ictx->iops_total_limit=atoi(iops_total_res.c_str());
	  ictx->total_iops_bucket_max=ictx->iops_total_limit*1.5;
	  
	  /*string config_oid= string(ictx->name)+string(".rbd");
	  librados::bufferlist flow_res;
  	  ictx->data_ctx.getxattr(config_oid,  "flow", flow_res);
	  librados::bufferlist iops_res;
  	  ictx->data_ctx.getxattr(config_oid,  "iops", iops_res);
	  ictx->max_limit=atoi(flow_res.c_str());
	  ictx->limit_iops=atoi(iops_res.c_str());*/
	  //char data[9];
      //sprintf(data,"%d\n",ictx->limit_iops);
      //::write(handle,data,32);
}

extern "C" int rbd_open(rados_ioctx_t p, const char *name, rbd_image_t *image,
			const char *snap_name)
{
  //if((handle=open("/root/ceph/11.log",O_WRONLY|O_CREAT|O_TRUNC,0777))==-1)
  ...
  //shiqi add search map
  pthread_mutex_init(&ictx->rdlock,NULL);
  pthread_mutex_init(&ictx->wtlock,NULL);
  
  ictx->bps_bucket_rd_max=1000000000;
  ictx->bps_bucket_wr_max=1000000000;
  ictx->iops_bucket_rd_max=1000000000;
  ictx->iops_bucket_wr_max=1000000000;
  ictx->total_bps_bucket_max=1000000000;
  ictx->total_iops_bucket_max=1000000000;
  ictx->bucket=0;
  ictx->iops_rdbucket=0;
  ictx->iops_rwbucket=0;
  ictx->total_bps_bucket=0;
  ictx->total_iops_bucket=0;
  ictx->last_time=0;
  ictx->iops_read_last_time=0;
  ictx->iops_write_last_time=0;
  ictx->rdbucket=0;
  ictx->read_last_time=0;

  watch_notify_test_cb(0,0,ictx);
  uint64_t mycookie = 0;
  string config_oid= ictx->header_oid;
  r= rados_watch(p, config_oid.c_str(),0, &ictx->watch_hanle, watch_notify_test_cb,ictx);
  }
  tracepoint(librbd, open_image_exit, r);
  return r;
}
...
extern "C" int rbd_close(rbd_image_t image)
{
  ...
  //shiqi add
  string config_oid=ctx->header_oid;
  //rados_unwatch(ctx->data_ctx, config_oid.c_str(), ctx->watch_hanle);
  ctx->data_ctx.unwatch(config_oid.c_str(),ctx->watch_hanle);
  //shiqi add
...
extern "C" ssize_t rbd_write(rbd_image_t image, uint64_t ofs, size_t len,
			     const char *buf)//----------------------------------------------------do_write() image就是我说的usr
{
  librbd::ImageCtx *ictx = (librbd::ImageCtx *)image;
